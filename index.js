...
function scramjetPluginsFor(tab) {
  ...
  // Keep links and form submissions inside the Scramjet browser.
  const catchEscapedLinks = new $scramjetUtils.CatchEscapedLinksPlugin((value) => {
    const escapedUrl = normalizeScramjetUrl(value);

    // Never build /undefined. Stay on the current tab when no valid URL exists.
...
function goTo(tab, url, pushHist) {
  if (!url || url === 'undefined' || url === '/undefined' || String(url).includes('fromUrl=%2Fundefined')) {
    url = START;
  }
  tab.url = url;
...
function navigateInput(raw) {
  ...
  if (!tab || !input || input === 'undefined') return;
  ...
  const url = looksLikeUrl(input) ? toUrl(input) : engine.url(input);
  if (!isBrokenUrl(url)) goTo(tab, url);
}
...
function renderPageContent(tab) {
  ...
  if (tab.url === GAMES_PAGE) {
    mountContent(page, buildGamesPage(tab));
    return;
  }
  if (tab.url === AI_PAGE) {
    mountContent(page, buildAIPage());
    return;
  }
  if (tab.url === MOVIES_PAGE) {
    mountContent(page, buildMoviesPage(tab));
    return;
  }
  if (tab.url === MUSIC_PAGE) {
    mountContent(page, buildMusicPage(tab));
    return;
  }
  if (tab.url === APPS_PAGE) {
    mountContent(page, buildAppsPage(tab));
    return;
  }
...
function buildStartPage(tab) {
  ...
  shell.className = 'bb-home-shell';

  const logo = document.createElement('div');
  logo.className = 'bb-logo';
  logo.innerHTML = `<img src="/icon.png" alt="Berri logo"><span>Berri</span>`;
  shell.appendChild(logo);

  const subtitle = document.createElement('div');
  subtitle.className = 'bb-home-subtitle';
  subtitle.textContent = 'your browser, your world';
  ...
  form.addEventListener('submit', event => {
    ...
    navigateInput(form.querySelector('input').value);
  });
  ...
  const routes = [
    ['Games', GAMES_PAGE, '/icons/gamepad.svg'],
    ['AI', AI_PAGE, '/icons/sparkles.svg'],
    ['Movies', MOVIES_PAGE, '/icons/film.svg'],
    ['Music', MUSIC_PAGE, '/icons/music.svg'],
    ['Add', START, '/icons/plus.svg'],
    ['All Apps', APPS_PAGE, '/icons/grid.svg']
  ];

  const grid = document.createElement('div');
  grid.className = 'bb-route-grid';

  routes.forEach(([label, route, icon]) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'bb-route-tile';
    button.innerHTML = `<span class="bb-route-icon"><img class="bb-real-icon" src="${icon}" alt=""></span><span class="bb-route-label">${label}</span>`;
  ...
  cards.className = 'bb-home-cards';
  cards.innerHTML = `
    <article class="bb-home-card">
      <div class="bb-home-card-top"><div class="bb-home-card-avatar">B</div><div><div class="bb-home-card-name">Berri</div><div class="bb-home-card-role">Fast browser</div></div></div>
      <p>Search the web, open internal pages, and keep everything inside one browser.</p>
    </article>
    <article class="bb-home-card">
      <div class="bb-home-card-top"><div class="bb-home-card-avatar">G</div><div><div class="bb-home-card-name">Games</div><div class="bb-home-card-role">berri://g</div></div></div>
      <p>Jump straight into the full Berri game library.</p>
    </article>
    <article class="bb-home-card">
      <div class="bb-home-card-top"><div class="bb-home-card-avatar">A</div><div><div class="bb-home-card-name">Berri AI</div><div class="bb-home-card-role">berri://ai</div></div></div>
      <p>Ask questions, write, code, and create without leaving Berri.</p>
    </article>
    <article class="bb-home-card">
      <div class="bb-home-card-top"><div class="bb-home-card-avatar">M</div><div><div class="bb-home-card-name">Media</div><div class="bb-home-card-role">Movies and music</div></div></div>
      <p>Your movie and music sections are ready to be connected.</p>
    </article>
    <article class="bb-home-card">
      <div class="bb-home-card-top"><div class="bb-home-card-avatar">S</div><div><div class="bb-home-card-name">Settings</div><div class="bb-home-card-role">Make it yours</div></div></div>
...
function buildAppsPage(tab) {
  const wrap = document.createElement('div');
  wrap.className = 'bb-apps-page';

  const header = document.createElement('div');
  header.className = 'bb-apps-header';
  header.innerHTML = `
    <h2>All Apps</h2>
    <p>berri://apps — all of your saved bookmarks and shortcuts</p>
  `;
  wrap.appendChild(header);

  const grid = document.createElement('div');
  grid.className = 'bb-apps-grid';

  bookmarks.forEach((bookmark, index) => {
    const card = document.createElement('button');
    card.type = 'button';
    card.className = 'bb-app-card';

    const iconUrl = faviconUrl(bookmark.url);
    card.innerHTML = `
      <button class="bb-app-card-remove" type="button" title="Remove">✕</button>
      <span class="bb-app-card-icon" style="${bookmark.color ? `background:${bookmark.color}` : ''}">
        <img src="${escapeAttr(iconUrl)}" alt=""
          onerror="this.style.display='none';this.parentElement.textContent='${escapeHtml(faviconLetter(bookmark.name))}'">
      </span>
      <span class="bb-app-card-label">${escapeHtml(bookmark.name)}</span>
    `;

    card.addEventListener('click', event => {
      if (event.target.classList.contains('bb-app-card-remove')) {
  ...
  add.type = 'button';
  add.className = 'bb-app-card bb-app-card-add';
  add.innerHTML = `
    <span class="bb-app-card-icon">
      <img class="bb-real-icon" src="/icons/plus.svg" alt="">
    </span>
    <span class="bb-app-card-label">Add Bookmark</span>
  `;
  add.addEventListener('click', openAddBookmark);
...
const BERRI_CINEMETA_BASE = 'https://v3-cinemeta.strem.io';

async function berriCinemeta(path) {
  const response = await fetch(`${BERRI_CINEMETA_BASE}${path}`);

  if (!response.ok) {
    throw new Error(`Movie catalog request failed (${response.status})`);
  }
...
function berriMovieCard(movie, onOpen) {
  const card = document.createElement('button');
  card.type = 'button';
  card.className = 'bb-movie-card';

  const poster = berriMoviePoster(movie);

  card.innerHTML = `
    ${
      poster
        ? `<img src="${escapeAttr(poster)}" alt="${escapeAttr(movie.name || movie.title || '')} poster" loading="lazy">`
        : `<div class="bb-movies-loading" style="aspect-ratio:2/3">No poster</div>`
    }
    <span class="bb-movie-card-info">
      <span class="bb-movie-title">${escapeHtml(movie.name || movie.title || 'Untitled')}</span>
      <span class="bb-movie-meta">${escapeHtml(berriMovieYear(movie))}</span>
    </span>
  `;
...
function berriUniqueMovies(movies) {
  ...
  return (movies || []).filter(movie => {
    const key = movie?.id || `${movie?.name || movie?.title || ''}-${berriMovieYear(movie)}`;
...
// Each source keeps the original URL behavior. Only the visible labels are animal names.
const BERRI_EMBED_SOURCES = [
  {
    ...
    urlImdb: imdb => `https://vidsrc.to/embed/movie/${imdb}`
  },
  {
    id: 'embedsu',
    label: 'Elk',
    ...
    urlImdb: imdb => `https://embed.su/embed/movie/${imdb}`
  },
  {
    id: 'vidsrcme',
    label: 'Tiger',
    ...
    urlImdb: imdb => `https://vidsrc.me/embed/movie?imdb=${imdb}`
  },
  {
    id: 'twoembed',
    label: 'Wolf',
    ...
    urlImdb: imdb => `https://www.2embed.cc/embed/${imdb}`
  },
  {
    id: 'multiembed',
    label: 'Falcon',
    ...
    urlImdb: imdb => `https://multiembed.mov/directstream.php?video_id=${imdb}`
  },
  {
    id: 'autoembed',
    label: 'Panda',
    ...
    urlImdb: imdb => `https://autoembed.co/movie/imdb/${imdb}`
  },
  {
    id: 'vidsrcxyz',
    label: 'Lion',
    note: 'Source 7',
    url: id => `https://vidsrc.xyz/embed/movie?tmdb=${id}`,
    urlImdb: imdb => `https://vidsrc.xyz/embed/movie?imdb=${imdb}`
  },
  {
    id: 'vidlink',
    label: 'Otter',
    ...
    urlImdb: imdb => `https://vidlink.pro/movie/${imdb}`
  },
  {
    id: 'embedsoap',
    label: 'Cobra',
    note: 'Source 9',
    url: id => `https://www.embedsoap.net/embed/movie/?id=${id}`,
    urlImdb: imdb => `https://www.embedsoap.net/embed/movie/?id=${imdb}`
  },
  {
    id: 'smashystream',
    label: 'Shark',
    note: 'Source 10',
    url: id => `https://player.smashystream.com/movie/${id}`,
    urlImdb: imdb => `https://player.smashystream.com/movie/${imdb}`
  },
  {
    id: 'superembed',
    label: 'Fox',
    note: 'Source 11',
    url: id => `https://superembed.stream/embed/movie?tmdb=${id}`,
    urlImdb: imdb => `https://superembed.stream/embed/movie?imdb=${imdb}`
  },
  {
    id: 'moviesapi',
    label: 'Bear',
    ...
    urlImdb: imdb => `https://moviesapi.club/movie/${imdb}`
  }
];

function berriBuildSourcePanel(context) {
  const { modalCard } = context;

  const dock = document.createElement('section');
  dock.className = 'bb-player-bottom';

  dock.innerHTML = `
    <div class="bb-player-bottom-label">Sources</div>
    <div class="bb-source-dock-scroll">
      ${BERRI_EMBED_SOURCES.map((source, index) => `
        <button
          type="button"
          class="bb-source-pill ${index === 0 ? 'active' : ''}"
          data-source-id="${source.id}"
        >
          <span class="bb-source-pill-name">${escapeHtml(source.label)}</span>
          <span class="bb-source-pill-note">${escapeHtml(source.note)}</span>
        </button>
      `).join('')}
    </div>
    <div class="bb-source-remember">
      <span>Remember</span>
      <button type="button" class="bb-source-toggle" aria-pressed="false"></button>
    </div>
  `;

  const showSource = sourceId => {
    dock.querySelectorAll('.bb-source-pill').forEach(item => {
      item.classList.toggle('active', item.dataset.sourceId === sourceId);
    });

    modalCard.querySelectorAll('[data-source-view]').forEach(view => {
      const active = view.dataset.sourceView === sourceId;
      view.hidden = !active;

      if (active && !view.querySelector('iframe')) {
        const src = view.dataset.embedSrc;
        if (src) {
          view.innerHTML = `
            <iframe
              class="bb-movie-video"
              src="${escapeAttr(src)}"
              title="${escapeAttr(sourceId)} movie source"
    ...
    if (dock.querySelector('.bb-source-toggle')?.classList.contains('on')) {
      localStorage.setItem('berri_movie_last_source_v1', sourceId);
    }
  };

  dock.querySelectorAll('.bb-source-pill').forEach(button => {
    button.addEventListener('click', () => showSource(button.dataset.sourceId));
  });

  dock.querySelector('.bb-source-toggle')?.addEventListener('click', event => {
    const toggle = event.currentTarget;
    toggle.classList.toggle('on');
    toggle.setAttribute(
      'aria-pressed',
      toggle.classList.contains('on') ? 'true' : 'false'
    );
  });

  const lastSource = localStorage.getItem('berri_movie_last_source_v1');
  const chosen = BERRI_EMBED_SOURCES.some(source => source.id === lastSource)
    ? lastSource
    : BERRI_EMBED_SOURCES[0].id;

  requestAnimationFrame(() => showSource(chosen));
  return dock;
}

function buildMoviesPage(tab) {
  const wrap = document.createElement('div');
  wrap.className = 'bb-movies-page';

  wrap.innerHTML = `
    <section class="bb-movies-hero">
      <div class="bb-movies-hero-content">
        <div class="bb-movies-brand">Berri Movies</div>
        <h1>Find your next movie.</h1>
        <p>
          Search a large movie catalog with no API key, then open the animal
          source selector inside the movie player.
        </p>
        <div class="bb-movies-actions">
          <button type="button" class="bb-movie-btn primary" data-movie-search-focus>
            Search Movies
          </button>
        </div>
      </div>
    ...
    <div class="bb-movies-toolbar">
      <form class="bb-movies-search">
        <span>⌕</span>
        <input type="search" placeholder="Search any movie title..." autocomplete="off">
      </form>
      <button type="button" class="bb-movies-key-btn" data-home-catalog>
        Browse
      </button>
    </div>
    ...
    <div class="bb-movie-modal" aria-hidden="true">
      <div class="bb-movie-modal-card"></div>
    </div>
  `;

  const content = wrap.querySelector('.bb-movies-content');
  const searchForm = wrap.querySelector('.bb-movies-search');
  const searchInput = searchForm.querySelector('input');
  const modal = wrap.querySelector('.bb-movie-modal');
  const modalCard = wrap.querySelector('.bb-movie-modal-card');

  let restoreWindowOpen = null;

  const enableMoviePopupGuard = () => {
    if (!settings.moviePopupBlocker || restoreWindowOpen) return;

    const originalWindowOpen = window.open;

    window.open = function blockedMoviePopup() {
      return null;
    };
  ...
  const closeModal = () => {
    if (restoreWindowOpen) restoreWindowOpen();
  ...
  const openMovie = async movie => {
    ...
    modalCard.innerHTML = '<div class="bb-movies-loading">Loading movie…</div>';

    let details = movie;

    try {
      if (movie.id) {
        const result = await berriCinemeta(`/meta/movie/${encodeURIComponent(movie.id)}.json`);
        details = result.meta || movie;
      }
    } catch {
      details = movie;
    }

    const imdb = details.id || movie.id || '';
    const poster = details.background || details.poster || movie.poster || '';
    const title = details.name || movie.name || 'Movie';
    const year = berriMovieYear(details);
    const description = details.description || 'No description is available for this movie.';

    modalCard.innerHTML = `
      <section class="bb-player-shell">
        <header class="bb-player-top">
          <button type="button" class="bb-player-close bb-movie-close">←</button>
          <div class="bb-player-meta">
            <div class="bb-player-kicker">Movie</div>
            <div class="bb-player-heading">${escapeHtml(title)}</div>
            <div class="bb-player-sub">${escapeHtml(year)}${imdb ? ` · ${escapeHtml(imdb)}` : ''}</div>
          </div>
        </header>

        <section class="bb-player-stage">
          <div class="bb-player-screen">
            ${BERRI_EMBED_SOURCES.map(source => `
              <div
                data-source-view="${source.id}"
                data-embed-src="${escapeAttr(source.urlImdb ? source.urlImdb(imdb) : source.url(imdb))}"
                hidden
              >
                <div class="bb-embed-placeholder" style="height:100%;display:grid;place-items:center;color:#bca8b2;font:600 12px var(--font-body)">
    ...
    const sourcePanel = berriBuildSourcePanel({ modalCard });
    const shell = modalCard.querySelector('.bb-player-shell');
    const desc = modalCard.querySelector('.bb-player-description');
    if (shell && desc) shell.insertBefore(sourcePanel, desc);

    modalCard.querySelector('.bb-movie-close')?.addEventListener('click', closeModal);
  };
  ...
  const renderRows = rows => {
    ...
    rows.forEach(([title, movies]) => {
      const unique = berriUniqueMovies(movies);
      if (!unique.length) return;

      const section = document.createElement('section');
      section.className = 'bb-movie-row';
      section.innerHTML = `
        <h2>${escapeHtml(title)}</h2>
        <div class="bb-movies-count">${unique.length} movies</div>
      `;

      const grid = document.createElement('div');
      grid.className = 'bb-movie-grid';

      unique.forEach(movie => {
        grid.appendChild(berriMovieCard(movie, openMovie));
      });
    ...
    if (!content.querySelector('.bb-movie-row')) {
      content.innerHTML = '<div class="bb-movies-empty">No movies were found.</div>';
    }
  };

  const fetchCatalog = async extra => {
    const suffix = extra ? `/${extra}` : '';
    const result = await berriCinemeta(`/catalog/movie/top${suffix}.json`);
    return result.metas || [];
  };
  ...
  const renderHome = async () => {
    ...
    try {
      ...
      const [top, ...genreRows] = await Promise.all([
        fetchCatalog(''),
        ...genres.map(genre =>
          fetchCatalog(`genre=${encodeURIComponent(genre)}`).catch(() => [])
        )
      ]);

      renderRows([
        ['Popular Movies', top],
        ...genres.map((genre, index) => [genre, genreRows[index]])
      ]);
    } catch (error) {
      content.innerHTML = `
        <div class="bb-movies-empty">
          The catalog could not load.<br>${escapeHtml(error.message)}
        </div>
      `;
    }
  };
  ...
  searchForm.addEventListener('submit', async event => {
    ...
    try {
      const result = await berriCinemeta(
        `/catalog/movie/top/search=${encodeURIComponent(query)}.json`
      );

      renderRows([[`Results for "${query}"`, result.metas || []]]);
    } catch (error) {
      content.innerHTML = `
        <div class="bb-movies-empty">
          Search failed.<br>${escapeHtml(error.message)}
        </div>
      `;
    }
  });

  wrap.querySelector('[data-movie-search-focus]')?.addEventListener('click', () => {
    searchInput.focus();
    searchInput.scrollIntoView({ behavior: 'smooth', block: 'center' });
  });

  wrap.querySelector('[data-home-catalog]')?.addEventListener('click', renderHome);
...
function buildGamesPage(tab) {
  const wrap = document.createElement('div');
  wrap.className = 'bb-games-native-page';

  const gamesId = `bbNativeLuminGames-${tab.id}`;

  wrap.innerHTML = `
    <div class="bb-games-native-shell">
      <div id="${gamesId}" class="bb-games-native-host">
        <div class="bb-route-loading">Loading games…</div>
      </div>
    </div>
  `;

  requestAnimationFrame(() => {
    const container = wrap.querySelector(`#${gamesId}`);
    ...
    const findGameCards = () => {
      const candidates = [...container.querySelectorAll('a, button, [role="button"]')]
        ...
        .filter(element => {
          ...
          return !['random', 'all', 'favorites', 'recent'].includes(text);
        });
    ...
    const enhanceNativeUi = () => {
      ...
      const randomButton = buttons.find(button =>
        (button.textContent || '').trim().toLowerCase().includes('random')
      );

      if (randomButton && randomButton.dataset.berriDice !== 'true') {
        randomButton.dataset.berriDice = 'true';
        randomButton.classList.add('bb-native-random-dice');

        const label =
          [...randomButton.childNodes]
            .find(node => node.nodeType === Node.TEXT_NODE && node.textContent.trim())
            ?.textContent.trim() || 'Random';

        randomButton.innerHTML = `${diceIcon}<span>${escapeHtml(label)}</span>`;
      }
      ...
      if (toolbarCandidate) {
        let count = toolbarCandidate.querySelector('.bb-native-games-count');

        if (!count) {
          count = document.createElement('span');
          count.className = 'bb-native-games-count';
          toolbarCandidate.appendChild(count);
        }
    ...
    try {
      ...
      const observer = new MutationObserver(() => {
        clearTimeout(container._berriNativeTimer);
        container._berriNativeTimer = setTimeout(enhanceNativeUi, 100);
      });
      ...
      setTimeout(enhanceNativeUi, 400);
      setTimeout(enhanceNativeUi, 1200);
      setTimeout(enhanceNativeUi, 2600);
    } catch (error) {
      console.error('Lumin failed:', error);
      container.innerHTML =
...
function buildMusicPage(tab) {
  const wrap = document.createElement('div');
  wrap.className = 'bb-music-page';

  const frameId = `bbMusicFrame-${tab.id}`;
  ...
  wrap.innerHTML = `
    <header class="bb-music-header">
      ...
      <div class="bb-music-actions">
        <button type="button" class="bb-music-reload">Reload</button>
        <button type="button" class="bb-music-open">Open as Browser Tab</button>
      </div>
    </header>

    <div class="bb-music-notice">
      Monochrome is a third-party service. Availability, accounts, playback,
      advertisements, and song licensing are controlled by that service.
    </div>
    ...
    <section class="bb-music-frame-shell">
      ...
      <div class="bb-music-error" hidden>
        <h3>Monochrome could not load inside the Music page</h3>
        <p>Open it as a regular Berri browser tab instead.</p>
        <button type="button">Open Monochrome</button>
      </div>
    </section>
  `;

  requestAnimationFrame(async () => {
    const iframe = wrap.querySelector(`#${frameId}`);
    const errorView = wrap.querySelector('.bb-music-error');
    const reloadButton = wrap.querySelector('.bb-music-reload');
    const openButton = wrap.querySelector('.bb-music-open');
    const errorOpenButton = errorView.querySelector('button');

    const openAsBrowserTab = () => {
      goTo(tab, 'https://monochrome.tf');
    };

    openButton.addEventListener('click', openAsBrowserTab);
    errorOpenButton.addEventListener('click', openAsBrowserTab);

    const loadMonochrome = async () => {
      ...
      const musicTab = {
        ...
        title: 'Monochrome',
        hist: ['https://monochrome.tf'],
        histIdx: 0
      };

      try {
        await openThroughScramjet(musicTab, iframe);
      } catch (error) {
        console.error('Monochrome music test failed:', error);
        iframe.hidden = true;
...
function buildAIPage() {
  ...
  frame.className = 'bb-ai-route-frame';
  frame.src = 'https://berri-app-2-1.vercel.app/';
  frame.title = 'Berri AI';
  frame.setAttribute(
    'allow',
...
function buildPlaceholderPage(title, icon, message) {
  const wrap = document.createElement('div');
  wrap.className = 'bb-internal-page';

  wrap.innerHTML = `
    <div class="bb-internal-header">
      <h2>${escapeHtml(title)}</h2>
      <p>berri://${escapeHtml(title.toLowerCase())}</p>
    </div>
    <div class="bb-placeholder">
      <div>
        <div class="bb-placeholder-icon">${icon}</div>
...
function buildSettingsPage(tab) {
  ...
  movieSection.className = 'bb-settings-section';
  movieSection.innerHTML = `
    <h4>Movies</h4>
    <label class="bb-setting-toggle-row">
      <span>
        <strong>Pop-up blocker</strong>
        <small>Blocks movie players from opening extra tabs or pop-up windows.</small>
      </span>
      <button
        type="button"
        class="bb-setting-switch ${settings.moviePopupBlocker ? 'on' : ''}"
        aria-pressed="${settings.moviePopupBlocker ? 'true' : 'false'}"
      ></button>
    </label>
    <div class="bb-setting-status">
      Automatically ${settings.moviePopupBlocker ? 'ON' : 'OFF'}
    </div>
  `;

  const moviePopupSwitch = movieSection.querySelector('.bb-setting-switch');
  const moviePopupStatus = movieSection.querySelector('.bb-setting-status');

  moviePopupSwitch.addEventListener('click', () => {
    settings.moviePopupBlocker = !settings.moviePopupBlocker;
    saveSettings();

    moviePopupSwitch.classList.toggle('on', settings.moviePopupBlocker);
    moviePopupSwitch.setAttribute(
      'aria-pressed',
      settings.moviePopupBlocker ? 'true' : 'false'
    );
    moviePopupStatus.textContent =
      `Automatically ${settings.moviePopupBlocker ? 'ON' : 'OFF'}`;
  });
...
const safeGoto = goto && goto !== 'undefined' && goto !== '/undefined' && !goto.includes('fromUrl=%2Fundefined');
if (safeGoto) {
...
document.querySelectorAll('.bb-side-route[data-route]').forEach(button => {
  button.addEventListener('click', () => {
    const tab = activeTab();
    if (!tab) return;
    goTo(tab, button.dataset.route);
  });
});
