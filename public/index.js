...
function addHistoryEntry(url) {
  if (url === START || url === HISTORY_PAGE || url === SETTINGS_PAGE || url === CHAT_PAGE || url === DMS_PAGE) return;
...
function renderPageContent(tab) {
  ...
  if (tab.url === DMS_PAGE) {
    if (typeof window.buildBerriDmsPage === 'function') {
      mountContent(page, window.buildBerriDmsPage(tab));
    }
    return;
  }
...
document.getElementById('bbChatBtn').addEventListener('click', () => {
  const tab = activeTab();
  if (tab) goTo(tab, CHAT_PAGE);
});
document.getElementById('bbDmsBtn').addEventListener('click', () => {
  const tab = activeTab();
  if (tab) goTo(tab, DMS_PAGE);
});
